 ...
 */
public class StakingWithdrawUnbonded {
	...
	public static void main(String[] args) throws Exception {
		...
		try (PolkadotApi client = PolkadotApi.newBuilder().subscriptionAdapter(adapter).build()) {
			...
			System.out.println("------");
			System.out.println("Withdraw unbonded : for " + numSlashingSpans + " slashing spans of " + controller);

			// prepare call, and sign with sender Secret Key within the context
			StakingRequests.WithdrawUnbondedTransfer withdrawUnbondedTransfer = StakingRequests.withdrawUnbonded()
					.runtime(metadata)
					.from(controller)
			...
			ByteData req = withdrawUnbondedTransfer.encodeRequest();
