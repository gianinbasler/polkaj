 ...
 */
public class StakingSetController {
	...
	public static void main(String[] args) throws Exception {
		...
		try (PolkadotApi client = PolkadotApi.newBuilder().subscriptionAdapter(adapter).build()) {
			...
			System.out.println("------");
			System.out.println("Set controller : " + " for " + alice + " to " + controller);

			// prepare call, and sign with sender Secret Key within the context
			StakingRequests.SetControllerTransfer setControllerTransfer = StakingRequests.setController()
					.runtime(metadata)
					.from(alice)
			...
			ByteData req = setControllerTransfer.encodeRequest();
